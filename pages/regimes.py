...
def app():
    ...
    st.markdown("<h1 style='text-align: center; color: black; font-size : 25px;'>Récupération des Régimes</h1>", unsafe_allow_html=True)
    st.divider()
    ...
    if  df_idap.empty or df_control.empty:
        ...
        output = utils.recuperation__regimes(idap_data=df_idap, control_data=df_control)
        ...
        st.download_button(label="📥 Télécharger au format .xlsx", data=df_to_save, file_name='Récupération des régimes IDAP pour SCORE.xlsx')
