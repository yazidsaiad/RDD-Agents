...
def app():
    ...
    st.markdown("<h1 style='text-align: center; color: black; font-size : 25px;'>Récupération des choix des Agents</h1>", unsafe_allow_html=True)
    st.divider()
    ...
    if  df_idap.empty or df_control.empty:
        ...
        col1_, col2_ = st.columns([0.6, 0.4])
        with col1_:
        ...
        output = utils.recuperation__des__choix(idap_data=df_idap, control_data=df_control)
